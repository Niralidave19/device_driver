...
void GPIO_Init(gpio_config_t gpio){
	...
	pGPIOx->gpio_moder = ~(pGPIOx->gpio_moder | (3<<(gpio.pin_number * 2)));
	...
	pGPIOx->gpio_ospeedr = ~(pGPIOx->gpio_ospeedr | (3<<(gpio.pin_number * 2)));
	...
	pGPIOx->gpio_pupdr = ~(pGPIOx->gpio_pupdr | (3<<(gpio.pin_number * 2)));
	pGPIOx->gpio_pupdr = pGPIOx->gpio_pupdr | temp;

	/*set output type register*/
	temp = 0;
	temp = temp | (gpio.op_type << (gpio.pin_number *2));
	pGPIOx->gpio_otyper = ~(pGPIOx->gpio_otyper | (1<<(gpio.pin_number)));
	...
	if(gpio.pin_number <= 7){
		...
		pGPIOx->gpio_aflr = ~(pGPIOx->gpio_aflr | (15<<(gpio.pin_number)));
		...
		temp = 0;
		temp = temp | (gpio.alt_func << (gpio.pin_number *4));
		pGPIOx->gpio_afrh = ~(pGPIOx->gpio_afrh | (15<<(gpio.pin_number)));
